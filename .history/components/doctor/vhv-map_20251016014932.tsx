...
export function VhvMap({ vhvs }: Props) {
  ...
  // Build markers and highlight for district selection
  useEffect(() => {
    if (!leafletLoaded) return
    const L = (window as any).L
    if (!L) return
    ...
    try {
      ...
      if (districtFilter !== ALL_DISTRICTS) {
        const anchor = getDistrictAnchor(districtFilter)
        const circle = L.circle(anchor, { radius: AREA_RADIUS_M, color: colorForDistrict(districtFilter), weight: 2, fillOpacity: 0.05 })
        hl.addLayer(circle)
      }

      for (const group of districtGroups) {
        const marker = L.circleMarker(group.anchor, { radius: 10, color: group.color, fillColor: group.color, fillOpacity: 0.85 })
        const tooltipLabel = `${group.displayName} (${group.items.length})`
        marker.bindTooltip(tooltipLabel, { direction: "top", offset: [0, -10], permanent: false })

        const popupEl = document.createElement("div")
        popupEl.className = "space-y-2"
        // header
        ...
        badge.textContent = `${group.items.length} VHV${group.items.length > 1 ? "s" : ""}`
        header.appendChild(title)
        header.appendChild(badge)
        popupEl.appendChild(header)

        const listWrap = document.createElement("div")
        listWrap.className = "max-h-64 overflow-y-auto pr-1"
        const listInner = document.createElement("div")
        listInner.className = "space-y-2"
        for (const v of group.items) {
          const item = document.createElement("div")
          item.className = "rounded border border-border bg-background/60 p-2"
          const nameEl = document.createElement("div")
          nameEl.className = "font-medium leading-tight"
          nameEl.textContent = getVhvLabel(v)
          item.appendChild(nameEl)
          if (v.phone) {
            const p = document.createElement("div")
            p.className = "text-sm text-muted-foreground"
            p.textContent = `Phone: ${v.phone}`
            item.appendChild(p)
          }
          if (v.email) {
            const e = document.createElement("div")
            e.className = "text-sm text-muted-foreground"
            e.textContent = `Email: ${v.email}`
            item.appendChild(e)
          }
          if (v.district || group.displayName) {
            const base = document.createElement("div")
            base.className = "text-sm"
            base.innerHTML = `Base area: <span class=\"font-medium\">${v.district || group.displayName}</span>`
            item.appendChild(base)
          }
          if (v.status) {
            const s = document.createElement("div")
            s.className = "text-xs text-muted-foreground"
            s.textContent = `Status: ${v.status}`
            item.appendChild(s)
          }
          listInner.appendChild(item)
        }
        listWrap.appendChild(listInner)
        popupEl.appendChild(listWrap)

        marker.bindPopup(popupEl)
        lg.addLayer(marker)
      }
    } catch (err) {
      // ignore errors during DOM-based popup build
      // console.error("leaflet render error", err)
    }
  }, [districtGroups, districtFilter, leafletLoaded])
