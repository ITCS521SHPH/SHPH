...
const normalizeIntakePayloadToFormData = (payload: any | null) => {
  ...
  const fullName = [patientBasics.firstName, patientBasics.lastName]
    .filter(Boolean)
    .join(" ")
    .trim();
  ...
  return {
    ...
    patientConcerns: toDisplayString(
      vhvNotes.patientConcerns || symptoms.chiefComplaint || ""
    ),
...
export function VHVDashboard() {
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "Detected hardcoded user ID, clearing localStorage and redirecting to login"
      );
      clearCurrentUser();
      router.push("/login");
    }
  }, [currentUser?.id, router]);
  const [selectedPatientForReview, setSelectedPatientForReview] =
    useState<any>(null);
  ...
  const [reviewIntakeStatus, setReviewIntakeStatus] = useState<string | null>(
    null
  );
  const submittableStatuses = useMemo(() => ["DRAFT", "CHANGES_REQUESTED"], []);
  const [showDataForm, setShowDataForm] = useState(false);
  const [selectedPatientForForm, setSelectedPatientForForm] =
    useState<any>(null);
  ...
  useEffect(() => {
    const fetchEmergencyCount = async () => {
      if (currentUser?.id) {
        try {
          const count = await emergencyApi.getActiveCount(
            currentUser.id,
            UserRole.VHV
          );
  ...
  const { data: allAssignmentsForVHV } = useApiData(
    getAllAssignmentsForVHV,
    []
  );
  ...
  const isAreaPlaceholder = (patient: any) => {
    ...
    const fn = (patient.firstName || (patient as any).first_name || "")
      .toString()
      .trim();
    return fn === "Area Task" || fn === "Area";
  };
  const visibleAssignments = (assignedPatients ?? []).filter(
    (a: any) => !isAreaPlaceholder(a.patient)
  );
  ...
  const filteredAssignments = useMemo(() => {
    if (assignedFilter === "active") {
      return (visibleAssignments ?? []).filter(
        (assignment: any) => !hasSubmittedIntake(assignment.patient)
      );
    }
    if (assignedFilter === "completed") {
      return (visibleAssignments ?? []).filter((assignment: any) =>
        hasSubmittedIntake(assignment.patient)
      );
    }
  ...
  const handleOpenDataForm = async (patient: any) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error occurred";
      alert(
        `Failed to create intake submission: ${errorMessage}. Please try again.`
      );
  ...
  const handleContinueDataForm = async (patient: any, intakeId: string) => {
    try {
      ...
      if (!patient || !patient.id) {
        console.error(
          "Cannot continue data form: Invalid patient object",
          patient
        );
        alert("Error: Invalid patient data. Please refresh and try again.");
        return;
      ...
      if (offlineData && offlineData.completedSections) {
        ...
        console.log(
          "Restored completed sections:",
          offlineData.completedSections
        );
      }
  ...
  const handleOpenPatientReview = async (patient: any, intake: any) => {
    try {
      let normalizedFormData = intake?.payload
        ? normalizeIntakePayloadToFormData(intake.payload)
        : null;
        ...
        if (intake?.id) {
          try {
            ...
            if (intakeResponse?.payload) {
              normalizedFormData = normalizeIntakePayloadToFormData(
                intakeResponse.payload
              );
            }
  ...
  const handleFormComplete = async () => {
    if (!selectedPatientForForm) {
      console.warn(
        "No patient selected when trying to complete data collection"
      );
      return;
    }
  ...
  const handleCompleteDataCollection = async (patient: any) => {
    ...
    try {
      ...
      const offlineData = patientId
        ? await getOfflineFormData(patientId)
        : null;

      let intakeId = currentIntakeId || offlineData?.intakeId || null;
      let intakeStatus: string | null = null;
      let normalizedFormData = offlineData?.formData
      ...
      setReviewIntakeStatus(statusToUse);
      setCanSubmitReview(
        submittableStatuses.includes(statusToUse) || !intakeStatus
      );
      setShowReviewPage(true);
    } catch (error) {
  ...
  const handleSubmitTaskForm = async (formData: Record<string, any>) => {
    ...
    try {
      console.log("[v0] Submitting task form:", {
        taskId: selectedTaskForForm.id,
        formData,
      });
  ...
  const handleCompleteTask = async (taskId: string) => {
    try {
      ...
      console.log(
        `Task "${task.title}" completed. Undo available for 10 seconds.`
      );
    } catch (error) {
  ...
  const visibleTasks = useMemo(() => {
    ...
    const patientDistrictById = new Map<string, string | undefined>(
      ((allAssignmentsForVHV ?? []) as any[])
        .map(
          (a: any) =>
            [a.patientId || a.patient?.id, a.patient?.district] as const
        )
        .filter(([pid]) => !!pid)
    );
  ...
  const activePatientList = useMemo(
    () =>
      patientTasksAll.filter(
        (task: any) => normStatus(task.status) !== "completed"
      ),
    [patientTasksAll]
  );
  ...
  const activePatients = visibleAssignments.filter(
    (assignment: any) => assignment.status === "active"
  );
  const completedVisits = visibleAssignments.filter(
    (assignment: any) => assignment.patient?.lastVisit
  ).length;
  ...
  const pendingTasks = visibleTasks.filter(
    (t: any) => normStatus(t.status) === "pending"
  ).length;
  const inProgressTasks = visibleTasks.filter(
    (t: any) => normStatus(t.status) === "in_progress"
  ).length;
  const completedTasks = visibleTasks.filter(
    (t: any) => normStatus(t.status) === "completed"
  ).length;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex flex-col gap-1">
                <h1 className="text-lg sm:text-xl font-bold leading-tight">
                  Data Collection – {selectedPatientForForm.firstName}{" "}
                  {selectedPatientForForm.lastName}
                </h1>
                <p className="text-sm text-muted-foreground">
                  Complete the structured data collection form
                </p>
              </div>
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex w-full items-start justify-end gap-2 md:w-auto md:items-center">
            ...
            <CardContent>
              <div
                className={`text-xl md:text-2xl font-bold ${
                  activeEmergencyCount > 0 ? "text-red-600" : ""
                }`}
              >
                ...
                expandedPatient
                  ? (() => {
                      const patient = assignedPatients?.find(
                        (a: any) => a.patient?.id === expandedPatient
                      )?.patient;
                      return patient
              ...
              <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0 gap-3">
                ...
                <div className="w-full sm:w-40">
                  <Select
                    ...
                    onValueChange={(v) =>
                      setAssignedFilter(v as "all" | "active" | "completed")
                    }
                  >
              ...
              <CardContent className="space-y-4">
                {patientsLoading ? (
                  ...
                  filteredAssignments.map((assignment: any) => {
                    ...
                    const latestIntake = patient.intakeSubmissions?.[0];
                    const dataStatusBadge = getIntakeReviewBadge(
                      latestIntake?.status
                    );
                            ...
                            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                              {(() => {
                                ...
                                const canContinue =
                                  ...
                                  latestIntake.status === "CHANGES_REQUESTED";
                        ...
                        {tasksLoading ? (
                          ...
                          activePatientList.map((task: any) => {
                            ...
                            const assignmentForTask = visibleAssignments.find(
                              (assignment: any) =>
                                assignment.patientId === task.patientId ||
                                assignment.patient?.id === task.patientId
                            );
                            const patient = assignmentForTask?.patient;
                            const hasForm =
                              task.description &&
                              task.description.includes("<FORM_SCHEMA>");
                                    ...
                                    <div className="flex items-center gap-2">
                                      {undoableActions[task.id] && (
                                        <Button
                                          ...
                                          size="sm"
                                          onClick={() =>
                                            handleUndoCompleteTask(task.id)
                                          }
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="date"
                  value={
                    selectedPatient.dob ? selectedPatient.dob.split("T")[0] : ""
                  }
                  onChange={(e) =>
                    setSelectedPatient({
                      ...selectedPatient,
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedPatient({ ...selectedPatient, district: value })
                  }
                >
                  <SelectTrigger>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete {selectedPatient?.firstName}{" "}
              {selectedPatient?.lastName} from the system. This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
